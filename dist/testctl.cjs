...
function loadConfig(projectDir) {
  const path = (0, import_node_path.join)(projectDir, "testctl.yaml");
  if (!(0, import_node_fs.existsSync)(path)) return { stacks: {} };
  let parsed;
  try {
    parsed = (0, import_yaml.parse)((0, import_node_fs.readFileSync)(path, "utf8"));
  } catch (e) {
...
function isFlutterDir(dir) {
  const pubspec = (0, import_node_path2.join)(dir, "pubspec.yaml");
  if (!(0, import_node_fs2.existsSync)(pubspec)) return false;
  const text = (0, import_node_fs2.readFileSync)(pubspec, "utf8");
...
function readPkg(dir) {
  ...
  try {
    return JSON.parse((0, import_node_fs2.readFileSync)(pkgPath, "utf8"));
  } catch {
    return null;
  }
}
...
function isElectronDir(dir) {
  const pkg = readPkg(dir);
  return !!(pkg && allDeps(pkg).electron);
}
function isNextDir(dir) {
  const pkg = readPkg(dir);
...
function hasPlaywrightConfig(dir) {
  return (0, import_node_fs2.existsSync)((0, import_node_path2.join)(dir, "playwright.config.ts")) || (0, import_node_fs2.existsSync)((0, import_node_path2.join)(dir, "playwright.config.js")) || (0, import_node_fs2.existsSync)((0, import_node_path2.join)(dir, "playwright.config.mjs"));
}
function hasFlutterIntegration(dir) {
  if (!(0, import_node_fs2.existsSync)((0, import_node_path2.join)(dir, "pubspec.yaml"))) return false;
  try {
    return (0, import_node_fs2.statSync)((0, import_node_path2.join)(dir, "integration_test")).isDirectory();
  } catch {
    return false;
  }
}
function isE2eDir(dir) {
  const deps = allDeps(readPkg(dir) || {});
  if (deps["@playwright/test"] || hasPlaywrightConfig(dir)) return true;
  return hasFlutterIntegration(dir);
}
function e2eFramework(dir) {
  const deps = allDeps(readPkg(dir) || {});
...
function walk(root, dir, depth, acc) {
  ...
  for (const e of entries) {
    ...
    walk(root, (0, import_node_path3.join)(dir, e.name), depth + 1, acc);
  }
}
function discoverTargets(root, config = {}, onlyStack = null) {
  const cfg = config.stacks || {};
  const found = [];
  walk(root, root, 0, found);
  const targets = [];
  const labelFor = (dir) => {
    const r = (0, import_node_path3.relative)(root, dir);
    return r === "" ? null : r;
  };
...
function lastRunRecord(results, timestamp) {
  return {
    timestamp,
    results: (results || []).map((r) => ({
      ...
      errored: !!r.errored,
      error: r.error || null,
      // `present` (consumed by explain's groupFailures) and `coverage` (consumed by
      // cmdContext) are carried so this record is a strict superset of every reader's needs.
      present: !!r.present,
      coverage: r.coverage ?? null,
...
function formatDigest(record) {
  ...
  lines.push(totalFail === 0 && redStacks === 0 ? "All recorded stacks were green." : `${totalFail} failing test(s) across ${redStacks} stack(s). (recalled, not re-run)`);
  return lines.join("\n");
}
function saveLastRun(projectDir, results, timestamp) {
  try {
    const tdir = (0, import_node_path5.join)(projectDir, ".testctl");
    if (!(0, import_node_fs5.existsSync)(tdir)) (0, import_node_fs5.mkdirSync)(tdir, { recursive: true });
    const gi = (0, import_node_path5.join)(tdir, ".gitignore");
    if (!(0, import_node_fs5.existsSync)(gi)) (0, import_node_fs5.writeFileSync)(gi, "*\n");
    (0, import_node_fs5.writeFileSync)((0, import_node_path5.join)(tdir, "last-run.json"), JSON.stringify(lastRunRecord(results, timestamp)));
  } catch {
  }
}
function loadLastRun(projectDir) {
  try {
    return JSON.parse((0, import_node_fs5.readFileSync)((0, import_node_path5.join)(projectDir, ".testctl", "last-run.json"), "utf8")) || null;
  } catch {
    return null;
  }
}
...
function findFrappeApp(dir, depth = 0) {
  ...
  for (const e of entries) {
    ...
    const found = findFrappeApp((0, import_node_path6.join)(dir, e.name), depth + 1);
...
function listSites(bench) {
  try {
    return (0, import_node_fs6.readdirSync)((0, import_node_path6.join)(bench, "sites")).filter((n) => !NON_SITE.has(n) && !n.startsWith(".")).filter((n) => {
      try {
        return (0, import_node_fs6.statSync)((0, import_node_path6.join)(bench, "sites", n)).isDirectory();
      } catch {
        return false;
      }
    });
...
function scanProject(cwd, homeDir) {
  ...
  if (hasFrappeMarker(cwd)) {
    ...
    let sites = [];
    try {
      const benches = (0, import_node_fs6.readdirSync)(homeDir, { withFileTypes: true }).filter((e) => e.isDirectory() && e.name.startsWith("frappe-bench")).map((e) => (0, import_node_path6.join)(homeDir, e.name));
      for (const b of benches) {
        if ((0, import_node_fs6.existsSync)((0, import_node_path6.join)(b, "apps", app))) {
          benchPath = b;
          sites = listSites(b);
          break;
        }
...
function readVersion() {
  const bases = [process.argv[1] || "", process.cwd()];
  for (const base of bases) {
    const dir = base ? (0, import_node_path7.dirname)(base) : "";
    for (const p of [(0, import_node_path7.join)(dir, "..", "package.json"), (0, import_node_path7.join)(dir, "package.json")]) {
      try {
        const v = JSON.parse((0, import_node_fs7.readFileSync)(p, "utf8")).version;
        if (v) return v;
      } catch {
      }
    }
...
function runDoctor() {
  ...
  return { version: readVersion(), node, tools, readyStacks };
}
function formatDoctor(report) {
  const n = report.node;
  const lines = ["testctl doctor", "\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"];
  if (report.version) lines.push(`  testctl v${report.version}`);
...
// lib/report.mjs
var LABELS = { frappe: "Frappe", flutter: "Flutter", electron: "Electron", nextjs: "Next.js", supabase: "Supabase" };
function fmtTime(ms) {
...
function isUnder(f, dir) {
  const nf = (0, import_node_path8.resolve)(f);
  const nd = (0, import_node_path8.resolve)(dir);
  return nf === nd || nf.startsWith(nd + import_node_path8.sep);
}
function selectChangedTargets(targets, changedAbsFiles, projectDir) {
  return targets.filter((t) => {
    if (t.notice) return true;
    if (!t.path) return true;
    const abs = (0, import_node_path8.resolve)(projectDir, t.path);
...
function gitToplevel(dir) {
  ...
  return top ? (0, import_node_path8.resolve)(top) : null;
}
...
function changedInRepo(root, ref) {
  ...
  const add = (out) => {
    for (const line of (out || "").split("\n")) {
      const f = line.trim();
      if (f) set.add((0, import_node_path8.resolve)(root, f));
    }
  };
...
function unconfiguredChangedNote(targets, changedAbsFiles) {
  ...
  for (const t of targets) {
    ...
    const abs = (0, import_node_path8.resolve)(t.dir);
...
function watchProject(dir, onChange, { debounceMs = 300 } = {}) {
  let timer = null;
  return (0, import_node_fs8.watch)(dir, { recursive: true }, (_event, filename) => {
...
async function runFrappe(cfg) {
  ...
  const logDir = (0, import_node_fs9.mkdtempSync)((0, import_node_path9.join)((0, import_node_os.tmpdir)(), "testctl-frappe-"));
  const logPath = (0, import_node_path9.join)(logDir, "frappe.log");
  let logBuf = "";
...
${run.proc.stdout || ""}${run.proc.stderr || ""}`;
      ...
      const xmlPath = (0, import_node_path9.join)(logDir, `${safeName(unit.value)}.xml`);
...
${proc.stdout || ""}${proc.stderr || ""}`;
      ...
      if ((0, import_node_fs9.existsSync)(xmlPath)) xmlText = (0, import_node_fs9.readFileSync)(xmlPath, "utf8");
    }
    if (xmlText) {
  ...
  if (cfg.coverage && !remote) {
    for (const p of [(0, import_node_path9.join)(benchPath, "sites", "coverage.xml"), (0, import_node_path9.join)(benchPath, "coverage.xml")]) {
      try {
        if ((0, import_node_fs9.existsSync)(p)) {
          coverage = parseCoverageXml((0, import_node_fs9.readFileSync)(p, "utf8"));
          break;
        }
      } catch {
        coverage = null;
...
async function runFlutter(cfg) {
  ...
  const logDir = (0, import_node_fs10.mkdtempSync)((0, import_node_path10.join)((0, import_node_os2.tmpdir)(), "testctl-flutter-"));
  const logPath = (0, import_node_path10.join)(logDir, "flutter.log");
...
${proc.stdout || ""}${proc.stderr || ""}`;
  ...
  if (cfg.coverage) {
    try {
      const lcovPath = (0, import_node_path10.join)(cwd, "coverage", "lcov.info");
      if ((0, import_node_fs10.existsSync)(lcovPath)) coverage = parseLcov((0, import_node_fs10.readFileSync)(lcovPath, "utf8"));
    } catch {
      coverage = null;
    }
  }
...
async function runElectron(cfg) {
  ...
  const logDir = (0, import_node_fs11.mkdtempSync)((0, import_node_path11.join)((0, import_node_os3.tmpdir)(), "testctl-electron-"));
  const logPath = (0, import_node_path11.join)(logDir, "electron.log");
  let logBuf = `$ ${command} ${args.join(" ")} (cwd: ${cwd})
${proc.stdout || ""}${proc.stderr || ""}`;
  if (proc.error && proc.error.code !== "ENOBUFS") {
    (0, import_node_fs11.writeFileSync)(logPath, logBuf);
  ...
  if (cfg.coverage && !cfg.command) {
    try {
      const sumPath = (0, import_node_path11.join)(cwd, "coverage", "coverage-summary.json");
      if ((0, import_node_fs11.existsSync)(sumPath)) coverage = parseJestCoverageSummary((0, import_node_fs11.readFileSync)(sumPath, "utf8"));
    } catch {
      coverage = null;
    }
  }
...
async function runNextjs(cfg) {
  const start = Date.now();
  const logDir = (0, import_node_fs12.mkdtempSync)((0, import_node_path12.join)((0, import_node_os4.tmpdir)(), "testctl-nextjs-"));
  const logPath = (0, import_node_path12.join)(logDir, "nextjs.log");
  if (!cfg.vercelUrl) {
    (0, import_node_fs12.writeFileSync)(logPath, "No vercelUrl configured.\n");
...
`;
  ...
  (0, import_node_fs12.writeFileSync)(logPath, logBuf);
  return makeResult({
    stack: "nextjs",
    passed,
    failed,
...
async function runSupabase(cfg) {
  ...
  const logDir = (0, import_node_fs13.mkdtempSync)((0, import_node_path13.join)((0, import_node_os5.tmpdir)(), "testctl-supabase-"));
  const logPath = (0, import_node_path13.join)(logDir, "supabase.log");
...
async function runWeb(cfg) {
  ...
  const proc = await spawnAsync(command, args, { cwd });
  const logDir = (0, import_node_fs14.mkdtempSync)((0, import_node_path14.join)((0, import_node_os6.tmpdir)(), "testctl-web-"));
  const logPath = (0, import_node_path14.join)(logDir, "web.log");
  let logBuf = `$ ${command} ${args.join(" ")} (cwd: ${cwd})
${proc.stdout || ""}${proc.stderr || ""}`;
  if (proc.error && proc.error.code !== "ENOBUFS") {
    (0, import_node_fs14.writeFileSync)(logPath, logBuf);
  ...
  if (cfg.coverage && !cfg.command) {
    try {
      const sumPath = (0, import_node_path14.join)(cwd, "coverage", "coverage-summary.json");
      if ((0, import_node_fs14.existsSync)(sumPath)) coverage = parseJestCoverageSummary((0, import_node_fs14.readFileSync)(sumPath, "utf8"));
    } catch {
      coverage = null;
    }
  }
...
async function runE2e(cfg) {
  ...
  const proc = await spawnAsync(command, args, { cwd });
  const logDir = (0, import_node_fs15.mkdtempSync)((0, import_node_path15.join)((0, import_node_os7.tmpdir)(), "testctl-e2e-"));
  const logPath = (0, import_node_path15.join)(logDir, "e2e.log");
  let logBuf = `$ ${command} ${args.join(" ")} (cwd: ${cwd})
${proc.stdout || ""}${proc.stderr || ""}`;
  if (proc.error && proc.error.code !== "ENOBUFS") {
    (0, import_node_fs15.writeFileSync)(logPath, logBuf);
  ...
  let counts;
  try {
    counts = framework === "flutter-integration" ? parseFlutterJson(proc.stdout || "") : parsePlaywrightJson(proc.stdout || "");
  } catch (e) {
...
function walkFiles(dir, acc) {
  ...
  for (const e of entries) {
    ...
    const full = (0, import_node_path16.join)(dir, e.name);
    if (e.isDirectory()) walkFiles(full, acc);
    else if (e.isFile()) acc.push(full);
  }
}
function hashApp(absPath) {
  if (!(0, import_node_fs16.existsSync)(absPath)) return null;
  const files = [];
  ...
  for (const f of files) {
    try {
      h.update((0, import_node_path16.relative)(absPath, f));
      h.update("\0");
      h.update((0, import_node_fs16.readFileSync)(f));
      h.update("\0");
    } catch {
    }
  }
...
function loadCache(projectDir) {
  try {
    return JSON.parse((0, import_node_fs16.readFileSync)((0, import_node_path16.join)(projectDir, ".testctl", "cache.json"), "utf8")) || {};
  } catch {
    return {};
  }
}
function saveCache(projectDir, cache) {
  try {
    const tdir = (0, import_node_path16.join)(projectDir, ".testctl");
    if (!(0, import_node_fs16.existsSync)(tdir)) (0, import_node_fs16.mkdirSync)(tdir, { recursive: true });
    const gi = (0, import_node_path16.join)(tdir, ".gitignore");
    if (!(0, import_node_fs16.existsSync)(gi)) (0, import_node_fs16.writeFileSync)(gi, "*\n");
    (0, import_node_fs16.writeFileSync)((0, import_node_path16.join)(tdir, "cache.json"), JSON.stringify(cache));
  } catch {
  }
}
...
function cmdInit(projectDir, { ci = false } = {}) {
  ...
  if ((0, import_node_fs17.existsSync)(path)) {
    ...
    const detection = scanProject(projectDir, (0, import_node_os8.homedir)());
    (0, import_node_fs17.writeFileSync)(path, buildInitYaml(detection));
  ...
  if (ci === "gitlab") {
    const wfPath = (0, import_node_path17.join)(projectDir, ".gitlab-ci.yml");
    if ((0, import_node_fs17.existsSync)(wfPath)) {
      console.log(".gitlab-ci.yml already exists \u2014 leaving it untouched.");
    } else {
      (0, import_node_fs17.writeFileSync)(wfPath, buildGitlabYaml());
      console.log(`Created ${wfPath}`);
    }
  } else if (ci) {
    const wfDir = (0, import_node_path17.join)(projectDir, ".github", "workflows");
    const wfPath = (0, import_node_path17.join)(wfDir, "testctl.yml");
    if ((0, import_node_fs17.existsSync)(wfPath)) {
      console.log(".github/workflows/testctl.yml already exists \u2014 leaving it untouched.");
    } else {
      (0, import_node_fs17.mkdirSync)(wfDir, { recursive: true });
      (0, import_node_fs17.writeFileSync)(wfPath, buildWorkflowYaml());
      console.log(`Created ${wfPath}`);
    }
  }
  return 0;
...
async function cmdRun(projectDir, only, coverage = false, concurrency = 4, minCoverage = null, changed = null, quiet = false, cache = false, junitPath = null, sarifPath = null, retries = null, htmlPath = null, notifyUrl = null, mdPath = null) {
  ...
  const decisions = targets.map((t) => {
    if (useCache && t.path && !t.notice) {
      const h = hashApp((0, import_node_path17.resolve)(projectDir, t.path));
...
Exit code: ${code}`);
  ...
  if (junitPath) {
    try {
      (0, import_node_fs17.writeFileSync)((0, import_node_path17.resolve)(projectDir, junitPath), toJUnitXml(results));
    } catch (e) {
  ...
  if (sarifPath) {
    try {
      (0, import_node_fs17.writeFileSync)((0, import_node_path17.resolve)(projectDir, sarifPath), JSON.stringify(toSarif(results), null, 2));
    } catch (e) {
  ...
  if (htmlPath) {
    try {
      (0, import_node_fs17.writeFileSync)((0, import_node_path17.resolve)(projectDir, htmlPath), toHtml(results));
    } catch (e) {
  ...
  if (mdPath) {
    try {
      (0, import_node_fs17.writeFileSync)((0, import_node_path17.resolve)(projectDir, mdPath), toMarkdown(results));
    } catch (e) {
...
function cmdExplain(projectDir) {
  let parsed;
  try {
    parsed = JSON.parse((0, import_node_fs17.readFileSync)((0, import_node_path17.join)(projectDir, ".testctl", "last-run.json"), "utf8"));
  } catch {
...
function walkSrcFiles(dir, acc, depth = 0) {
  ...
  for (const e of entries) {
    ...
    const full = (0, import_node_path17.join)(dir, e.name);
...
function cmdContext(projectDir, only) {
  ...
  const lastByKey = {};
  try {
    const lr = JSON.parse((0, import_node_fs17.readFileSync)((0, import_node_path17.join)(projectDir, ".testctl", "last-run.json"), "utf8"));
  ...
  const apps = targets.map((t) => {
    ...
    app.untested = [];
    if (t.path) {
      const abs = (0, import_node_path17.resolve)(projectDir, t.path);
      const files = [];
      ...
      for (const tf of testFiles.slice(0, 60)) {
        try {
          testText += "\n" + (0, import_node_fs17.readFileSync)(tf, "utf8");
        } catch {
        }
      }
      const symbols = [];
      for (const sf of srcFiles.slice(0, 250)) {
        ...
        let txt = "";
        try {
          txt = (0, import_node_fs17.readFileSync)(sf, "utf8");
        } catch {
          continue;
        }
        for (const s of extractSymbols(txt, lang)) symbols.push({ ...s, file: (0, import_node_path17.relative)(projectDir, sf) });
      }
...
async function cmdWatch(projectDir, runOnce) {
  ...
  watchProject(projectDir, async () => {
    ...
    try {
      ...
      console.warn(`testctl: ${e.message}`);
    }
    console.log("\n\u{1F440} watching for changes \u2014 Ctrl-C to stop");
    running = false;
  });
...
function cmdPreflight(projectDir) {
  ...
  if (!fc) {
    ...
    let siteConfig = {};
    try {
      siteConfig = JSON.parse((0, import_node_fs17.readFileSync)((0, import_node_path17.join)(benchPath, "sites", site, "site_config.json"), "utf8"));
    } catch {
      siteConfig = {};
    }
    const appsWithBeforeTests = apps.filter((app) => {
      for (const p of [(0, import_node_path17.join)(benchPath, "apps", app, app, "hooks.py"), (0, import_node_path17.join)(benchPath, "apps", app, "hooks.py")]) {
        try {
          if (/^\s*before_tests\s*=/m.test((0, import_node_fs17.readFileSync)(p, "utf8"))) return true;
        } catch {
        }
      }
      return false;
    ...
    let devReqsOk = false;
    try {
      const proc = (0, import_node_child_process4.spawnSync)((0, import_node_path17.join)(benchPath, "env", "bin", "python"), ["-c", "import xmlrunner, coverage"], { encoding: "utf8" });
