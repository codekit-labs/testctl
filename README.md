...
`isolation-guard` proves the suite stays deterministic: it snapshots the identity set of rows/files
the test creates (scoped by a unique marker — making the check parallel-safe) before and after a test
and asserts no leftover, and it re-runs the suite under a verified different order (confirming the
permutation actually changed) and asserts the same result — reporting a leaking or order-dependent test
