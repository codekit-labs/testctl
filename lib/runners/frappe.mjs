...
export function buildRemoteBenchCommand(benchPath, site, app, xmlPath) {
...
function runSsh(ssh, remoteCommand) {
  ...
  const proc = spawnSync(inv.command, inv.args, {
    encoding: 'utf8',
...
export function runFrappe(cfg) {
  ...
  for (const app of apps) {
    ...
    if (remote) {
      ...
      runSsh(cfg.ssh, `rm -f ${remoteXml}`); // best-effort cleanup
    } else {
