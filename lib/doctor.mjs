...
// Best-effort: read the installed testctl version from package.json near the running script.
// Works in dev (bin/testctl.mjs → ../package.json) and prod (dist/testctl.cjs → ../package.json).
function readVersion() {
  const bases = [process.argv[1] || '', process.cwd()];
  for (const base of bases) {
    const dir = base ? dirname(base) : '';
    for (const p of [join(dir, '..', 'package.json'), join(dir, 'package.json')]) {
      try {
