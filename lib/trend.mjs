...
export function computeTrend(historyText, { window = 10 } = {}) {
  ...
  for (const key of order) {
    ...
    // A single-run series has no first half → passRatePrev=0, which would spuriously show ↑100%.
    // Treat it as flat: no directional inference is possible from one data point.
