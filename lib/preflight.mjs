...
export function frappePreflight(inputs = {}) {
  ...
  if (!configured) {
    return { ok: true, blockers: 0, checks: [
      { id: 'configured', label: 'No Frappe stack configured', ok: true, blocking: false, info: true,
        fix: 'Nothing to preflight — run `testctl init` to add a Frappe stack' },
    ] };
  }
  if (remote) {
    return { ok: true, blockers: 0, checks: [
      { id: 'remote', label: 'Remote (ssh) bench', ok: true, blocking: false, info: true,
...
export function formatPreflight(report, opts = {}) {
  ...
  // Info-only reports (not configured / remote) have nothing to run — skip the ready/blocker line.
